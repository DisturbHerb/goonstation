...
const Flavor = (props: FlavorArray, context) => {
  const { act } = useBackend<IceCreamData>(context);

  return (
    <Stack.Item width="50%">
      <Button fluid onClick={() => act('dispense_flavor', { flavor: props.flavorName })}>
        <Icon
...
const InsertedContainer = (_, context) => {
  ...
  return (
    <Section
      capitalize
      fill
      title={!data.containerData ? containerData.name : ""}
      buttons={
        <Stack fluid>
          {!!data.containerData && (
            <Stack.Item>
              <Button onClick={() => act('dispense_flavor', { flavor: 'beaker' })}>
                <Icon
