/datum/antagonist/incursion_operative
	id = ROLE_INCURSION
	display_name = "\improper Syndicate Incursion Operative"
	...
	New(datum/mind/new_owner)
		...
		if (istype(ticker.mode, /datum/game_mode/incursion))
			var/datum/game_mode/incursion/gamemode = ticker.mode
	...
	assign_objectives()
		ticker.mode.bestow_objective(src.owner, /datum/objective/specialist/incursion, src)
	...
	proc/assign_name()
		...
		// I guess we won't go with the funny operative names.
		// 	if (src.id == ROLE_INCURSION_COMMANDER)
...
/datum/antagonist/incursion_operative/commander
	...
	display_name = "\improper Syndicate Incursion Commander"
