...
/obj/machinery/medical/dialysis/New()
	. = ..()
	AddComponent(/datum/component/medical_device/transfuser/dialysis, reservoir = src)
	RegisterSignal(src, COMSIG_INFUSER_DRAW, PROC_REF(on_draw))
	RegisterSignal(src, COMSIG_DIALYSIS_PRE_INFUSION, PROC_REF(on_infusion))
	src.UpdateIcon()
...
/obj/machinery/medical/dialysis/update_icon(...)
	..()
	// if (!src.medical_device.active || src.is_disabled() || !src.medical_device.patient)
	if (src.is_disabled())
...
/obj/machinery/medical/dialysis/proc/on_draw(datum/component/medical_device/equipment, datum/reagents/reservoir)
	src.update_tubing(reservoir, "tubing-bad")

/obj/machinery/medical/dialysis/proc/on_infusion(datum/component/medical_device/equipment, datum/reagents/reservoir)
